...
if(isset($_POST['id_category_v1'])) {
    ...
    if($id_category_v1 !== 'none'){
        ...
        if(!empty($_SESSION['selected_product_category']['v1'])) {
            $data = pdo_query(
            'SELECT v2.*
                FROM category_v2 v2
                LEFT JOIN category_v1 v1
                ON v2.id_category_v1 = v1.id_category_v1
                WHERE v1.id_category_v1 = '. $_SESSION['selected_product_category']['v1']
            );

            if(!empty($data)) {
                foreach ($data as $row) {
                    $selected = false;
                    foreach($_SESSION['selected_product_category']['v2'] as $id_v2) {
